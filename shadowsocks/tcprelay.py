...
from shadowsocks import cryptor, obfs, eventloop, shell, common, lru_cache, version
...
class TCPRelayHandler(object):
    ...
    def _create_encryptor(self, config):
        try:
            self._encryptor = cryptor.Cryptor(config['password'],
                                                config['method'])
            return True
        except Exception:
                ...
                if self._encrypt_correct:
                    ...
                    if obfs_decode[1]:
                        ...
                        try:
                            ...
                            logging.error("aead decrypt exception from %s:%d maybe wrong password or encryption method" % (self._client_address[0], self._client_address[1]))
                            self.destroy()
                            return
#                         data = self._encryptor.decrypt(obfs_decode[0])
                    else:
