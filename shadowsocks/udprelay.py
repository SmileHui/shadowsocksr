...
from shadowsocks import cryptor, obfs, eventloop, lru_cache, common, shell
...
class UDPRelay(object):
    def __init__(self, config, dns_resolver, is_local, stat_callback=None, stat_counter=None):
        ...
        server_info.key = cryptor.encrypt_key(self._password, self._method)
    ...
    def _handle_server(self):
        ...
        if self._is_local:
            ...
            try:
                data, key, ref_iv = cryptor.decrypt_all(self._password,self._method,data)
            except Exception:
    ...
    def _handle_server_dns_resolved(self, error, remote_addr, server_addr, params):
        ...
        try:
            ...
            if self._is_local:
                key, ref_iv, m = cryptor.gen_key_iv(self._password, self._method)
                ...
                try:
                    data=cryptor.encrypt_all_m(key, ref_iv, m, self._method, data)
                except Exception:
    ...
    def _handle_client(self, sock):
        ...
        if not self._is_local:
            ...
            ref_iv = [cryptor.encrypt_new_iv(self._method)]
            self._protocol.obfs.server_info.iv = ref_iv[0]
            ...
            data, key, ref_iv = cryptor.decrypt_all(self._password,
                                                    self._method, data,
                                                    self._crypto_path)
            if not data:
                return
